...
//    for (int i = 0; i < pulsingCount; i++) {
    ...
    [self setUpAnimationWithDuring:animationDuring delay:0.0 rect:self.bounds maxScale:1.0];

    
    
    [self setUpAnimationWithDuring:animationDuring delay:0.5 rect:self.bounds maxScale:1.0];
//    }
...
- (void)setUpAnimationWithDuring:(float )during delay:(float )delay rect:(CGRect )rect maxScale:(float)scale{
        ...
        animationGroup.duration = during;
        ...
        opacityAnimation.values     = @[@(1.0),@(0.3),@(0.0)];
